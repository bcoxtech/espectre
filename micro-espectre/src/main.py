    ...
    while calibration_progress < config.CALIBRATION_BUFFER_SIZE:
        ...
        if frame:
            ...
            if calibration_progress % 100 == 0:
                ...
                print(
                    f"Collecting {calibration_progress}/{config.CALIBRATION_BUFFER_SIZE} packets... (pps:{pps}, TG:{tg_pps}, drop:{dropped})"
                )
            ...
            if timeout_counter >= max_timeout:
                print(
                    f"Timeout waiting for CSI packets (collected {calibration_progress}/{config.CALIBRATION_BUFFER_SIZE})"
                )
    ...
    if config.TRAFFIC_GENERATOR_RATE > 0:
        ...
        for tg_attempt in range(max_tg_retries):
            ...
            if tg_attempt < max_tg_retries - 1:
                print(
                    f"WARNING: Only {csi_received} CSI packets - restarting TG (attempt {tg_attempt + 2}/{max_tg_retries})"
                )
                traffic_gen.stop()
                time.sleep(1)
                traffic_gen.start(config.TRAFFIC_GENERATOR_RATE)
            ...
            if frame:
                ...
                if runtime_policy.should_evaluate(should_publish):
                    ...
                    if g_state.current_channel != 0 and packet_channel != g_state.current_channel:
                        print(
                            f"[WARN] WiFi channel changed: {g_state.current_channel} -> {packet_channel}, resetting detection buffer"
                        )
                        detector.reset()
                    ...
                    if should_publish:
                        ...
                        motion_metric = metrics.get(
                            "moving_variance", metrics.get("jitter", metrics.get("probability", 0))
                        )
                        ...
                        print(
                            f"{progress_bar} | pkts:{publish_counter} drop:{dropped_delta} pps:{pps} | "
                            f"mvmt:{motion_metric:.4f} thr:{threshold:.4f} | {state_str}"
                        )

                        mqtt_handler.publish_state(
                            motion_metric, effective_state, threshold, publish_counter, dropped_delta, pps
                        )
                        publish_counter = 0
