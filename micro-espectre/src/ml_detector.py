...
def normalize_features(features):
    ...
    if len(features) != len(FEATURE_MEAN):
        raise ValueError(f"Expected {len(FEATURE_MEAN)} features, got {len(features)}")
    normalized = []
...
class MLDetector(IDetector):
    ...
    def update_state(self):
        ...
        if not self.is_ready():
            return {"state": self._state, "probability": 0.0, "threshold": self._threshold}
        ...
        return {"state": self._state, "probability": self._current_probability, "threshold": self._threshold}
    ...
    def _extract_features(self):
        ...
        return extract_features_by_name(turb_list, len(turb_list), feature_names=FEATURE_NAMES)
