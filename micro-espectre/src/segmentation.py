...
class SegmentationContext:
    ...
    def __init__(
        ...
        if enable_lowpass:
            try:
                ...
                self.lowpass_filter = LowPassFilter(cutoff_hz=lowpass_cutoff, sample_rate_hz=100.0, enabled=True)
        ...
        if enable_hampel:
            try:
                ...
                self.hampel_filter = HampelFilter(window_size=hampel_window, threshold=hampel_threshold)
            except Exception as e:
    ...
    def compute_spatial_turbulence(csi_data, selected_subcarriers=None, use_cv_normalization=True):
        ...
        scratch = [0.0] * 64
        count = SegmentationContext._fill_amplitude_buffer(csi_data, selected_subcarriers, scratch)
        turbulence = SegmentationContext._turbulence_from_amplitude_buffer(scratch, count, use_cv_normalization)
    ...
    def _compute_spatial_turbulence_in_buffer(self, csi_data, selected_subcarriers=None):
        ...
        self._amplitude_count = self._fill_amplitude_buffer(csi_data, selected_subcarriers, self._amplitude_buffer)
    ...
    def calculate_spatial_turbulence(self, csi_data, selected_subcarriers=None, return_amplitudes=False):
        ...
        """
        turbulence = self._compute_spatial_turbulence_in_buffer(csi_data, selected_subcarriers)
