...
class TestMLInferenceAccuracy:
    ...
    def test_inference_matches_reference(self):
        ...
        for i in range(num_samples):
            ...
            assert error < INFERENCE_TOLERANCE, (
                f"Sample {i}: expected {expected:.6f}, got {result:.6f}, error {error:.6f}"
            )
    ...
    def test_all_samples_match(self):
        ...
        assert max_error < INFERENCE_TOLERANCE, f"Max error {max_error:.2e} exceeds tolerance {INFERENCE_TOLERANCE:.2e}"
    ...
    def test_output_range(self):
        ...
        for i in range(len(self.features)):
            ...
            assert 0.0 <= result <= ML_METRIC_SCALE, f"Sample {i}: output {result} outside [0, {ML_METRIC_SCALE}] range"
...
class TestMLInferencePerformance:
    ...
    def test_inference_speed(self):
        ...
        assert inferences_per_second > 100, f"Inference too slow: {inferences_per_second:.0f} inferences/sec"
