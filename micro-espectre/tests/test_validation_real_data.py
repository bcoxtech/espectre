...
def get_available_datasets():
    ...
    try:
        ...
        datasets.append(pytest.param((baseline_c3, movement_c3, 64, "C3"), id="c3_64sc"))
    ...
    try:
        ...
        datasets.append(pytest.param((baseline_c5, movement_c5, 64, "C5"), id="c5_64sc"))
    except FileNotFoundError:
        pass
    ...
    try:
        ...
        datasets.append(pytest.param((baseline_c6, movement_c6, 64, "C6"), id="c6_64sc"))
    except FileNotFoundError:
        pass
    ...
    try:
        ...
        datasets.append(pytest.param((baseline_esp32, movement_esp32, 64, "ESP32"), id="esp32_64sc"))
    except FileNotFoundError:
        pass
    ...
    try:
        ...
        datasets.append(pytest.param((baseline_s3, movement_s3, 64, "S3"), id="s3_64sc"))
    except FileNotFoundError:
        pass
...
def run_nbvi_calibration(
    baseline_packets, num_subcarriers, hint_band=None, mvs_window_size=None, use_cv_normalization=False
):
    """
...
class TestMVSDetectionRealData:
    ...
    def test_baseline_low_motion_rate(
        self,
        real_data,
        num_subcarriers,
        window_size,
        ...
        assert motion_rate < target_rate, (
            f"[{calibration_algorithm}] Baseline motion rate too high: {motion_rate:.1%} (target: <{fp_rate_target}%)"
        )

    def test_movement_high_motion_rate(
        self,
        real_data,
        num_subcarriers,
        window_size,
        ...
        assert motion_rate > min_recall_rate, (
            f"[{calibration_algorithm}] Movement motion rate too low: {motion_rate:.1%} (target: >{recall_target}%)"
        )

    def test_mvs_detector_wrapper(
        self,
        real_data,
        num_subcarriers,
        window_size,
        ...
        detector = MVSDetector(
            window_size=window_size, threshold=adaptive_threshold, selected_subcarriers=selected_band, track_data=True
        )
...
class TestFeatureSeparationRealData:
    ...
    def test_skewness_separation(self, baseline_amplitudes, movement_amplitudes):
        ...
        baseline_skew = [
            calc_skewness(list(r), len(r), float(np.mean(r)), float(np.std(r))) for r in baseline_amplitudes
        ]
        movement_skew = [
            calc_skewness(list(r), len(r), float(np.mean(r)), float(np.std(r))) for r in movement_amplitudes
        ]
    ...
    def test_turbulence_variance_separation(
        self, real_data, default_subcarriers, chip_type, use_cv_normalization, window_size
    ):
    ...
    def test_hampel_reduces_spikes(self, real_data, default_subcarriers):
        ...
        for pkt in all_packets:
            turb = calculate_spatial_turbulence(
                pkt["csi_data"], default_subcarriers, gain_locked=pkt.get("gain_locked", True)
            )
...
class TestPerformanceMetrics:
    ...
    def test_mvs_default_subcarriers(
        self,
        real_data,
        window_size,
        mvs_default_fp_rate_target,
        ...
        cal_ctx = SegmentationContext(window_size=window_size, threshold=1.0, enable_hampel=enable_hampel)
    ...
    def test_mvs_detection_accuracy(
        self,
        real_data,
        num_subcarriers,
        window_size,
    ...
    def test_ml_detection_accuracy(self, real_data, num_subcarriers, ml_fp_rate_target, ml_recall_target, chip_type):
        ...
        if baseline_eval_count > 0:
            assert pkt_fp_rate < ml_fp_rate_target, (
                f"ML FP Rate too high: {pkt_fp_rate:.1f}% (target: <{ml_fp_rate_target}%)"
            )
...
class TestFloat32Stability:
    ...
    def test_turbulence_float32_accuracy(self, real_data, default_subcarriers):
        ...
        assert max_rel_error < 0.001, f"Float32 turbulence error too high: {max_rel_error:.4%}"
    ...
    def test_csi_range_values_float32_stable(self):
        ...
        assert error_two_pass < 0.001, f"Two-pass error too high for CSI range: {error_two_pass:.4%}"
        assert error_single_pass < 0.001, f"Single-pass error too high for CSI range: {error_single_pass:.4%}"
...
class TestEndToEndWithCalibration:
    ...
    def test_band_calibration_produces_valid_band(
        self, real_data, num_subcarriers, calibration_algorithm, chip_type, default_subcarriers
    ):
        ...
        selected_band, adaptive_threshold = run_calibration(
            baseline_packets, num_subcarriers, calibration_algorithm, hint_band=default_subcarriers
        )
    ...
    def test_end_to_end_with_band_calibration_and_mvs(
        self,
        real_data,
        num_subcarriers,
        window_size,
        ...
        f1 = (
            2 * (precision / 100) * (recall / 100) / ((precision + recall) / 100) * 100
        ...
        assert recall > recall_target, (
            f"End-to-end Recall too low ({num_subcarriers} SC): {recall:.1f}% (target: >{recall_target}%)"
        )
        assert fp_rate < fp_rate_target, (
            f"End-to-end FP Rate too high ({num_subcarriers} SC): {fp_rate:.1f}% (target: <{fp_rate_target}%)"
        )
