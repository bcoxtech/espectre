...
from csi_utils import calculate_spatial_turbulence, HampelFilter, find_dataset, load_baseline_and_movement
from config import SEG_WINDOW_SIZE, SEG_THRESHOLD, HAMPEL_WINDOW, HAMPEL_THRESHOLD, LOWPASS_CUTOFF, DEFAULT_SUBCARRIERS
...
class ChebyshevFilter:
    ...
    def __init__(self, order=CHEBYSHEV_ORDER, cutoff=CHEBYSHEV_CUTOFF, ripple=CHEBYSHEV_RIPPLE, fs=SAMPLING_RATE):
        self.order = order
        self.cutoff = cutoff
        self.ripple = ripple
        self.fs = fs
...
class WaveletFilter:
    ...
    def __init__(self, wavelet=WAVELET_TYPE, level=WAVELET_LEVEL, threshold=WAVELET_THRESHOLD, mode=WAVELET_MODE):
    ...
    def filter(self, value):
        ...
        if len(denoised) > self.buffer_size:
            ...
            denoised = np.pad(denoised, (0, self.buffer_size - len(denoised)), mode="edge")
...
class FilterPipeline:
    ...
    def __init__(self, config):
        ...
        self.hampel = (
            HampelFilter(window_size=HAMPEL_WINDOW, threshold=HAMPEL_THRESHOLD) if config.get("hampel", False) else None
        )
...
def run_comparison_test(baseline_packets, movement_packets, num_packets=None, track_data=False):
    ...
    for name, config in configs.items():
        seg = FilteredStreamingSegmentation(
            window_size=WINDOW_SIZE, threshold=THRESHOLD, filter_config=config, track_data=track_data
        )
        ...
        for packet in baseline_to_process:
            ...
            turbulence = calculate_spatial_turbulence(csi_data, DEFAULT_SUBCARRIERS, gain_locked=gain_locked)
...
def plot_filter_effect(baseline_packets, movement_packets, num_packets=500):
    ...
    for name, config in filter_configs:
        ...
        for i in range(min(num_packets, len(baseline_packets))):
            ...
            turb = calculate_spatial_turbulence(csi_data, DEFAULT_SUBCARRIERS, gain_locked=gain_locked)
    ...
    for i, (name, data) in enumerate(results.items()):
        ...
        ax1.plot(time_baseline, data["baseline_mv"], color=colors[name], linewidth=1.0, alpha=0.8)
        ...
        for j, var in enumerate(data["baseline_mv"]):
            if var > THRESHOLD:
                ax1.axvspan(j / SAMPLING_RATE, (j + 1) / SAMPLING_RATE, alpha=0.3, color="red")
        ...
        ax2.plot(time_movement, data["movement_mv"], color=colors[name], linewidth=1.0, alpha=0.8)
        ...
        for j, var in enumerate(data["movement_mv"]):
            if var > THRESHOLD:
                ax2.axvspan(j / SAMPLING_RATE, (j + 1) / SAMPLING_RATE, alpha=0.2, color="green")
...
def plot_comparison(results, threshold):
    ...
    fig.suptitle("ESPectre - No Filter vs Top 3 Filters (by Score)", fontsize=14, fontweight="bold")
...
def optimize_filter_parameters(baseline_packets, movement_packets):
    ...
    for cutoff in cutoff_values:
        ...
        for packet in baseline_packets[:500]:
            csi_data, gain_locked = extract_csi_and_gain_locked(packet)
            seg.add_turbulence(calculate_spatial_turbulence(csi_data, DEFAULT_SUBCARRIERS, gain_locked=gain_locked))
...
def main():
    parser = argparse.ArgumentParser(
        description="ESPectre - Filtered Segmentation Test", formatter_class=argparse.RawDescriptionHelpFormatter
    )
    ...
    parser.add_argument("--optimize-filters", action="store_true", help="Optimize filter parameters")
    ...
    results = run_comparison_test(baseline_packets, movement_packets, num_packets=None, track_data=args.plot)
    ...
    for name in config_order:
        if name in results:
            result = results[name]
            print(
                f"{name:<20} {result['baseline_fp']:<5} {result['fp_rate']:<8.1f} "
    ...
    best_lowpass = max([(n, r) for n, r in results.items() if n in lowpass_filters], key=lambda x: x[1]["score"])
