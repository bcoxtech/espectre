...
class CSIReceiver:
    ...
    def _parse_packet(self, data: bytes) -> Optional[CSIPacket]:
        ...
        iq_raw = np.array(struct.unpack(f"<{iq_size}b", data[header_size : header_size + iq_size]), dtype=np.int8)
    ...
    def _wait_for_ready_state(self, quiet: bool = False) -> None:
        ...
        while True:
            try:
                ...
                packet_dict = {"csi_data": packet.iq_raw, "gain_locked": packet.gain_locked}
...
def get_dataset_stats() -> Dict[str, Any]:
    ...
    info = load_dataset_info()
    stats = {"labels": {}, "total_samples": 0, "total_packets": 0, "labels_count": 0}
...
def load_baseline_and_movement(
    baseline_file: str = None, movement_file: str = None, chip: str = "C6"
    ...
    if not movement_path.exists():
        raise FileNotFoundError(
            f"{movement_path} not found.\nCollect data using: ./me collect --label movement --duration 10"
        )
...
def test_mvs_configuration(
    ...
    if recall >= recall_target and fp_rate <= fp_target:
        ...
        score = -1_000_000.0 - (recall_target - recall) * 2_000.0 - fn_rate * 200.0 - fp_rate * 20.0 + precision
