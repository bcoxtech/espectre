...
from config import SEG_WINDOW_SIZE, SEG_THRESHOLD, HAMPEL_WINDOW, HAMPEL_THRESHOLD, DEFAULT_SUBCARRIERS
...
class StreamingSegmentationWrapper:
    ...
    def __init__(self, window_size=SEG_WINDOW_SIZE, threshold=1.0, track_data=False, enable_hampel=False):
...
def run_comparison(baseline_packets, movement_packets, track_data=False):
    ...
    for pkt in baseline_packets:
        seg.add_turbulence(
            calculate_spatial_turbulence(pkt["csi_data"], DEFAULT_SUBCARRIERS, gain_locked=pkt.get("gain_locked", True))
        )
    ...
    for pkt in baseline_packets:
        turb = calculate_turbulence_filtered_iq(
            pkt["csi_data"], hampel_I, hampel_Q, DEFAULT_SUBCARRIERS, gain_locked=pkt.get("gain_locked", True)
        )
    ...
    for pkt in baseline_packets:
        turb = calculate_turbulence_filtered_amplitudes(
            pkt["csi_data"], hampel_amps, DEFAULT_SUBCARRIERS, gain_locked=pkt.get("gain_locked", True)
        )
        seg.add_turbulence(turb)
    baseline_fp = seg.motion_packets
    baseline_data = {"moving_var": np.array(seg.moving_var_history)} if track_data else None
