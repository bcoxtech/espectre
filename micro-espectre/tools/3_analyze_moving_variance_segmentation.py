...
def select_subcarriers_nbvi(baseline_packets):
    ...
    with tempfile.NamedTemporaryFile(mode="wb", suffix="_nbvi_buffer.bin", delete=False) as tmp_file:
        temp_buffer = tmp_file.name
    nbvi_calibrator.BUFFER_FILE = temp_buffer

    try:
        calibrator = NBVICalibrator(buffer_size=len(baseline_packets))
...
def evaluate_subcarriers(
    ...
    external_adaptive_threshold=None,
):
    """
...
def plot_comparison(results, threshold):
    ...
    fig.suptitle(
        f"Subcarrier Selection Comparison - Window={WINDOW_SIZE}, Adaptive P{ADAPTIVE_PERCENTILE}x{ADAPTIVE_FACTOR}",
        fontsize=14,
        fontweight="bold",
    )
    ...
    for row, strategy in enumerate(strategies):
        ...
        ax1.set_title(
            f"{title_prefix}{strategy} - Baseline (FP={r['fp']}, FP Rate={r['fp_rate']:.1f}%)\nSC: {subcarriers}",
            fontsize=10,
        )
        ...
        ax2.set_title(
            f"{title_prefix}{strategy} - Movement (TP={r['tp']}, Recall={r['recall']:.1f}%, Prec={r['precision']:.1f}%)\nSC: {subcarriers}",
            fontsize=10,
        )
...
def print_comparison_summary(results, threshold):
    ...
    for strategy in ["Fixed", "NBVI"]:
        ...
        print(
            f"{marker}{strategy:<8} {r['fp']:<8} {r['tp']:<8} {r['recall']:<10.1f} {r['fp_rate']:<10.1f} {r['f1']:<10.1f} {time_str:<12}"
        )
...
def main():
        ...
        if args.use_test_dataset:
            try:
                test_path, baseline_packets, movement_packets, motion_start_packet, chip_name = load_test_dataset(
                    chip=chip, motion_start_packet=args.test_motion_start_packet
                )
                ...
                test_path, baseline_packets, movement_packets, motion_start_packet, chip_name = load_test_dataset(
                    chip=None, motion_start_packet=args.test_motion_start_packet
                )
    ...
    results = {}
    results["Fixed"] = evaluate_subcarriers(
        baseline_packets, movement_packets, fixed_subcarriers, THRESHOLD, WINDOW_SIZE, use_adaptive_threshold=True
    )
    results["Fixed"]["calibration_time_ms"] = 0.0  # No calibration for fixed

    results["NBVI"] = evaluate_subcarriers(
        baseline_packets,
        movement_packets,
        nbvi_subcarriers,
        THRESHOLD,
