...
def analyze_amplitudes_per_subcarrier(packets, name, num_subcarriers=64):
    ...
    for sc_idx in range(num_subcarriers):
        if amp_per_sc[sc_idx]:
            ...
            stats.append({"sc_idx": sc_idx, "mean": 0, "std": 0, "min": 0, "max": 0})

    return {"name": name, "stats": stats, "raw_amps": amp_per_sc, "num_subcarriers": num_subcarriers}
...
def calculate_spatial_turbulence(csi_data, selected_subcarriers, gain_locked=True):
    ...
    return SegmentationContext.compute_spatial_turbulence(
        csi_data, selected_subcarriers, use_cv_normalization=not bool(gain_locked)
    )
...
def analyze_turbulence_and_mvs(packets, name, selected_subcarriers, window_size):
    ...
    for pkt in packets:
        turb, amps = calculate_spatial_turbulence(
            pkt["csi_data"], selected_subcarriers, gain_locked=pkt.get("gain_locked", True)
        )
...
def main():
    ...
    for chip in chips_data:
        data = chips_data[chip]
        amp_stats[chip] = analyze_amplitudes_per_subcarrier(data["baseline"] + data["movement"], chip, data["num_sc"])
    ...
    for chip in chips_data:
        ...
        selected_means[chip] = np.mean(
            [amp_stats[chip]["stats"][sc]["mean"] for sc in selected_scs[chip] if sc < num_sc]
        )
    ...
    for key in turb_stats:
        data = turb_stats[key]
        print(
            f"  {data['name']:<20} {data['turb_mean']:>12.4f} {data['turb_std']:>12.4f} "
    ...
    for key in turb_stats:
        data = turb_stats[key]
        print(
            f"  {data['name']:<20} {data['mvs_mean']:>12.4f} {data['mvs_std']:>12.4f} "
    ...
    if args.plot:
        try:
            ...
            for chip in chip_list:
                ax.hist(
                    iq_stats[chip]["i_values"],
                    bins=50,
                    alpha=0.4,
                    label=f"{chip} I",
                    color=chip_colors.get(chip, "gray"),
                )
            ...
            ax.axvspan(
                min(DEFAULT_SUBCARRIERS), max(DEFAULT_SUBCARRIERS), alpha=0.2, color="yellow", label="Selected SC"
            )
            ...
            for chip in chip_list:
                ...
                if key in turb_stats:
                    ax.plot(
                        turb_stats[key]["turbulences"][:500], label=chip, alpha=0.7, color=chip_colors.get(chip, "gray")
                    )
            ...
            for chip in chip_list:
                ...
                if key in turb_stats:
                    ax.plot(
                        turb_stats[key]["mvs_values"][:500], label=chip, alpha=0.7, color=chip_colors.get(chip, "gray")
                    )
