...
def analyze_all_pairs_from_dataset_info() -> list:
    ...
    for baseline in baseline_entries:
        ...
        movement_start = datetime.fromisoformat(movement_by_name[movement_name]["collected_at"])
...
def print_summary(results: list):
    ...
    for r in results:
        ...
        print(
            f"{r['chip']:<6} {format_variance(baseline_var)} {format_variance(movement_var)} {ratio:>8.1f}x {status:<10}"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Analyze raw CSI data quality for all available datasets")
    parser.add_argument("--chip", type=str, help="Analyze only this chip type (e.g., C6, S3). Default: analyze all")
