...
def plot_constellation_comparison(
    baseline_packets, movement_packets, subcarriers, num_packets=50, offset=0, total_subcarriers=64
):
    """
    ...
    fig.suptitle(
        f"I/Q Constellation Diagrams - {total_subcarriers} SC Dataset - {sc_range}\n{num_packets} Packets (offset={offset})",
        fontsize=14,
        fontweight="bold",
    )
    ...
    for i, sc_idx in enumerate(subcarriers):
        ...
        ax3.scatter(
            I_vals, Q_vals, color=colors[i], alpha=0.7, s=30, label=f"SC {sc_idx}", edgecolors="black", linewidth=0.5
        )
    ...
    for sc_idx in subcarriers[: min(4, len(subcarriers))]:  # Show stats for first 4
        ...
        Q_std = np.std(Q_vals)
        print(
            f"  SC {sc_idx:2d}: I_std={I_std:6.2f}, Q_std={Q_std:6.2f}, "
...
def plot_single_subcarrier_grid(
    baseline_packets, movement_packets, subcarriers, num_packets=50, offset=0, total_subcarriers=64
):
    """
    ...
    fig.suptitle(
        f"Individual Subcarrier Constellations - {num_packets} Packets (offset={offset})",
        fontsize=14,
        fontweight="bold",
    )
...
Examples:
  ...
  python tools/8_plot_constellation.py --chip S3 --grid
    ...
    parser.add_argument("--packets", type=int, default=50, help="Number of contiguous packets to plot (default: 50)")
    parser.add_argument("--offset", type=int, default=0, help="Starting packet index (default: 0)")
    parser.add_argument(
        "--subcarriers",
        type=str,
        default=None,
    ...
    parser.add_argument("--grid", action="store_true", help="Use grid layout (one subplot per subcarrier)")
    ...
    try:
        baseline_packets, movement_packets = load_baseline_and_movement(
            baseline_file=baseline_file, movement_file=movement_file
        )
    except FileNotFoundError as e:
        print(f"Error: {e}")
        return
