...
def parse_hidden_layers(value):
    ...
    if not layers or any(layer <= 0 for layer in layers):
        raise argparse.ArgumentTypeError("hidden layers must contain one or more positive integers")
    return layers
...
def parse_positive_chip_boost(value):
    ...
    for part in text.split(","):
        ...
        if not item:
            continue
        if "=" not in item:
            raise argparse.ArgumentTypeError("positive chip boost must use CHIP=FACTOR entries, e.g. ESP32=1.2")
        ...
        try:
            ...
            raise argparse.ArgumentTypeError(f"invalid boost factor for {chip!r}: {factor!r}") from exc
        if factor_value <= 0.0:
            raise argparse.ArgumentTypeError("positive chip boost factors must be > 0")
...
def _resolve_counterpart_name(label, entry, dataset_info, max_delta_seconds=30 * 60):
    ...
    counterpart_field = "optimal_pair_movement_file" if label == "baseline" else "optimal_pair_baseline_file"
...
def extract_features(
    ...
    for source_file, file_packets in grouped.items():
        ...
        for pkt in file_packets:
            ...
            features = extract_features_by_name(turb_list, n, amplitudes=last_amplitudes, feature_names=feature_names)
...
def compute_mvs_guided_sample_weights(packets, tuning_map, window_size=SEG_WINDOW_SIZE):
    ...
    for source_file, file_packets in grouped.items():
        ...
        effective_threshold = confidence_factor * threshold + (1.0 - confidence_factor) * 1.0
...
def slice_sample_context(sample_context, indices):
    ...
    if sample_context is None:
        return None
    return {key: np.asarray(values)[indices] for key, values in sample_context.items()}
...
def build_model(hidden_layers=None, num_features=12, use_dropout=True, dropout_rate=0.2, seed=None):
    """
    ...
    model.add(
        tf.keras.layers.Dense(
            1,
            activation="sigmoid",
            kernel_initializer=tf.keras.initializers.GlorotUniform(seed=derive_seed(seed, len(hidden_layers), 0)),
            bias_initializer="zeros",
        )
    )
...
def train_model(
    ...
    if class_weight is None:
        ...
        if n_pos > 0 and n_neg > 0:
            ...
            class_weight = {0: n_total / (2 * n_neg), 1: n_total / (2 * n_pos)}
    ...
    if sample_weight is not None:
        X_t, X_v, y_t, y_v, sw_t, sw_v = _val_split(X, y, sample_weight, **split_kwargs)
    else:
    ...
    callbacks = [
        tf.keras.callbacks.EarlyStopping(
            monitor="val_loss", patience=DEFAULT_EARLY_STOP_PATIENCE, restore_best_weights=True, min_delta=1e-4
        ),
        tf.keras.callbacks.ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=DEFAULT_LR_PATIENCE, min_lr=1e-6),
    ]
    ...
    try:
        ...
        try:
            with tempfile.TemporaryDirectory() as saved_model_dir:
                ...
                tflite_model = convert_with(tf.lite.TFLiteConverter.from_saved_model(saved_model_dir))
        except Exception:
            ...
            tflite_model = convert_with(tf.lite.TFLiteConverter.from_concrete_functions([concrete_fn], export_module))
...
def export_micropython(model, scaler, output_path, seed=None, feature_names=None, scaler_mode=DEFAULT_SCALER_MODE):
    """
...
def export_cpp_weights(model, scaler, output_path, seed=None, feature_names=None, scaler_mode=DEFAULT_SCALER_MODE):
    """
...
'''
    ...
    for i in range(0, len(weights), 2):
        ...
        code += (
            f"constexpr float ML_W{layer_num}[{len(flat_weights)}] = {{"
    ...
    code += f"constexpr uint8_t ML_MODEL_LAYER_INPUT_SIZES[ML_MODEL_NUM_LAYERS] = {{{', '.join(input_sizes)}}};\n"
    code += f"constexpr uint8_t ML_MODEL_LAYER_OUTPUT_SIZES[ML_MODEL_NUM_LAYERS] = {{{', '.join(output_sizes)}}};\n"
    code += f"constexpr const float* ML_MODEL_WEIGHTS[ML_MODEL_NUM_LAYERS] = {{{', '.join(weight_names)}}};\n"
    code += f"constexpr const float* ML_MODEL_BIASES[ML_MODEL_NUM_LAYERS] = {{{', '.join(bias_names)}}};\n\n"
...
def run_shap_all_features(n_samples=100):
    ...
    # Calculate SHAP
    importance = calculate_shap_importance(model, X_scaled, actual_features, n_samples=n_samples)
    if importance:
...
def print_feature_importance(importance, title="Feature Importance (SHAP)", current_features=None):
    """
    ...
    if current_features:
        ...
        low_used = [f for f in sorted_features if f in current_features and importance[f] / total < 0.05]
        if top_unused:
...
def run_ablation_study(
    ...
    print(f"    F1: {baseline_f1:.2f}% (+/- {baseline_cv['f1_std']:.2f}%), blocked OOF={baseline_cv['oof_f1']:.2f}%\n")
    ...
    for i, feature_name in enumerate(feature_names):
        ...
        print(f"    F1: {f1:.2f}% ({direction} {delta:+.2f}%), blocked OOF={cv['oof_f1']:.2f}%\n")
    ...
    print(
        f"{'Removed Feature':<24} {'F1 (CV)':>14} {'OOF F1':>10} {'Delta':>10} {'Recall':>10} {'FP Rate':>10} {'Note':<12}"
    )
    print("-" * 85)
    ...
    bl = results[0]
    print(
        f"{'None (baseline)':<24} {bl['f1_mean']:>8.2f}% +/-{bl['f1_std']:.1f} "
    ...
    for r in sorted_results:
        ...
        print(
            f"{r['removed']:<24} {r['f1_mean']:>8.2f}% +/-{r['f1_std']:.1f} "
...
def train_all(
    ...
    sample_weights = compute_mvs_guided_sample_weights(all_packets, tuning_map, window_size=SEG_WINDOW_SIZE)
    ...
    if len(sample_weights) != len(X):
        print(f"Error: sample weights mismatch (weights={len(sample_weights)}, samples={len(X)}).")
        return 1, seed, None
    print("  Weight mode: context-aware + hard-positive mining (threshold fallback + per-file normalization)")
    print(
    ...
    if positive_chip_boost is not None:
        applied = [
            f"{chip}x{info['factor']:.2f} ({info['affected']} motion windows)" for chip, info in boost_summary.items()
        ]
    ...
    print(f"\n{min(DEFAULT_CV_FOLDS, unique_eval_groups)}-fold grouped CV by {DEFAULT_PRIMARY_GROUP_KEY}...")
    cv_start = perf_counter()
    with suppress_stderr():
    ...
    if feature_importance:
        importance = calculate_shap_importance(model, X_scaled, actual_feature_names, n_samples=shap_samples)
        if importance:
...
class StreamingEvaluator:
    ...
    def process_packet(self, csi_data):
        ...
        turb_list = self.context.turbulence_buffer[idx:] + self.context.turbulence_buffer[:idx]
...
def evaluate_long_gate(model, scaler, feature_names, threshold=0.5, chips=None):
    ...
    by_chip = {}
    for _, baseline_packets, movement_packets, _, chip, _ in get_available_long_test_datasets(chips=chips):
        by_chip[chip] = evaluate_split(
            model,
            scaler,
            feature_names,
...
def experiment_architectures(
    ...
    baseline_name = next(spec["name"] for spec in architectures if tuple(spec["layers"]) == baseline_layers)
    ...
    challengers = [
        item for item in sorted(screening_results, key=architecture_campaign_rank_key) if item["name"] != baseline_name
    ][:2]
    ...
    challenger_summaries = [
        item for item in sorted(seed_filter, key=aggregate_architecture_rank_key) if item["name"] != baseline_name
    ]
    ...
    promote_candidate = winner["name"] != baseline_name and architecture_candidate_beats_baseline(
...
""",
    ...
    parser.add_argument("--experiment", action="store_true", help="Run the FP-first MLP topology campaign")
    parser.add_argument(
        "--experiment-promote",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help=f"JSON output path for --experiment results (default: {DEFAULT_EXPERIMENT_OUTPUT})",
    )
    parser.add_argument(
        "--experiment-architectures",
        type=parse_architecture_sweep,
        default=None,
        help='Semicolon-separated hidden-layer specs for --experiment, e.g. "16,8;24,12;32,16;24;24,12,6"',
    )
    parser.add_argument("--seed", type=int, default=None, help="Use specific random seed for reproducible training")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f"Mini-batch size for Keras training (default: {DEFAULT_BATCH_SIZE})",
    )
    parser.add_argument(
        "--hidden-layers",
        type=parse_hidden_layers,
        default=None,
        help=f"Comma-separated hidden layer widths for the MLP (default: {','.join(map(str, DEFAULT_HIDDEN_LAYERS))})",
    )
    parser.add_argument(
        "--environment",
        type=str,
    ...
    parser.add_argument(
        "--positive-chip-boost",
        type=parse_positive_chip_boost,
        default=None,
    ...
    parser.add_argument("--ablation", action="store_true", help="Run ablation study (test removing each feature)")
