...
def _extract_motion_start_from_description(description):
    ...
    if not description:
        return None
    match = re.search(r"motion\s+starts\s+at\s+packet(?:\s+index)?(?:\s+n\.)?\s+(\d+)", description, re.IGNORECASE)
    if match:
...
def load_test_dataset(chip=None, motion_start_packet=None):
    ...
    if chip_upper:
        candidates = [entry for entry in test_entries if str(entry.get("chip", "")).upper() == chip_upper]
        if not candidates:
            raise FileNotFoundError(f"No test dataset found for chip {chip_upper} in dataset_info.json")
    else:
    ...
    if motion_start_packet is None:
        motion_start_packet = _extract_motion_start_from_description(str(selected.get("description", "")))

    if motion_start_packet is None:
        motion_start_packet = len(packets) // 2

    if motion_start_packet <= 0 or motion_start_packet >= len(packets):
        raise ValueError(f"Invalid motion start packet {motion_start_packet} for {len(packets)} packets")
...
def apply_config_filters(series):
    ...
    lowpass = (
        LowPassFilter(cutoff_hz=LOWPASS_CUTOFF, sample_rate_hz=100.0, enabled=True) if ENABLE_LOWPASS_FILTER else None
    )
    for value in series:
        out = float(value)
...
def compare_detection_methods(baseline_packets, movement_packets, subcarriers, window_size, threshold):
    ...
    for pkt in baseline_packets:
        ...
        methods["Turbulence"]["baseline"].append(
            calculate_spatial_turbulence(pkt["csi_data"], subcarriers, gain_locked=pkt.get("gain_locked", True))
        )
    ...
    if ML_AVAILABLE:
        ...
        ml_baseline = MLDetectorAdapter(
            window_size, ml_subcarriers, track_data=True, use_cv_normalization=use_cv_norm_ml
        )
        for pkt in baseline_packets:
...
def plot_comparison(
    ...
    for row, method_name in enumerate(method_names):
        ...
        ax_baseline.axhline(
            y=method_threshold, color="r", linestyle="--", linewidth=2, label=f"Threshold={method_threshold:.4f}"
        )
        ...
        ax_baseline.set_title(
            f"{title_prefix}{method_name} - Baseline (FP={fp}) [{time_info}]", fontsize=11, fontweight="bold"
        )
        ...
        ax_movement.set_title(
            f"{title_prefix}{method_name} - Movement (TP={tp}, R={recall:.0f}%, P={precision:.0f}%)",
            fontsize=11,
            fontweight="bold",
        )
...
def print_comparison_summary(
    ...
    for r in results:
        ...
        print(
            f"{marker} {r['name']:<13} {r['fp']:<8} {r['tp']:<8} {r['fn']:<8} "
...
def run_all_chips():
    ...
    for chip in chips:
        try:
            ...
            baseline_packets, movement_packets = load_baseline_and_movement(
                baseline_file=baseline_path, movement_file=movement_path, chip=chip
            )
        ...
        mvs_f1 = (
            2 * mvs_precision * mvs_recall / (mvs_precision + mvs_recall) if (mvs_precision + mvs_recall) > 0 else 0
        )
    ...
    for r in all_results:
        ...
        for detector, data in [("MVS", r["mvs"]), ("ML", r["ml"])]:
            ...
            print(
                f"{chip:<6} {marker}{detector:<8} {data['recall']:>9.1f}% {fp_rate:>9.1f}% {data['precision']:>9.1f}% {data['f1']:>9.1f}%"
            )
        print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compare detection methods (RSSI, Mean Amplitude, Turbulence, MVS, ML)"
    )
    ...
    try:
        if args.use_test_dataset:
            try:
                test_path, baseline_packets, movement_packets, motion_start_packet, chip_name, test_entry = (
                    load_test_dataset(chip=chip, motion_start_packet=args.test_motion_start_packet)
                )
            except FileNotFoundError:
                if chip_explicit:
                    raise
                ...
                test_path, baseline_packets, movement_packets, motion_start_packet, chip_name, test_entry = (
                    load_test_dataset(chip=None, motion_start_packet=args.test_motion_start_packet)
                )
    ...
    result = compare_detection_methods(baseline_packets, movement_packets, selected_subcarriers, WINDOW_SIZE, threshold)
    methods, mvs_baseline, mvs_movement, timing, ml_baseline, ml_movement, method_thresholds, results = result
